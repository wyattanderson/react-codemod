...
function isImportingReact(path, moduleName) {
  const {value} = path;
  return (
    value.source.value === moduleName &&
...
module.exports = function(file, api) {
  ...
  [
    ...
    let coreImportDeclaration;
    root
      .find(j.ImportDeclaration)
      .filter(p => isImportingReact(p, coreModuleName))
      .forEach(p => {
        const isDefault = p.node.specifiers.every((node) => {
          return node.type === 'ImportDefaultSpecifier';
        });
        if (isDefault) {
          if (p.node.specifiers.length !== 1) {
            throw new Error('Saw more than one default specifier.');
          }
    ...
    var coreName = coreImportDeclaration.value.specifiers[0].local.name;
    ...
    function insertRequire(name, path) {
      ...
      // TODO deal with non-default specifier
      coreImportDeclaration.insertAfter(j.importDeclaration(
